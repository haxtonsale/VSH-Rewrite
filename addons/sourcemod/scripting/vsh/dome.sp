...
static float g_flDomeSlowdown = 0.0;
...
void Dome_Init()
{
	...
	g_ConfigConvar.Create("vsh_dome_color_neu", "255 255 255 255", "Color of dome in RGBA if nobody owns the capture point");
	...
	g_ConfigConvar.Create("vsh_dome_speed_duration", "40", "How long it takes in second for dome to fully shrink, without any slowdown", _, true, 0.0);
	g_ConfigConvar.Create("vsh_dome_speed_end", "0.67", "Speed multiplier of dome when finish shrinking from start of shrink", _, true, 0.0, true, 1.0);
	g_ConfigConvar.Create("vsh_dome_speed_death", "1.0", "If attack team owns CP, how much slowdown multiplier to add on every death", _, true, 0.0, true, 1.0);
	g_ConfigConvar.Create("vsh_dome_speed_damage", "0.001", "If boss team owns CP, how much slowdown multiplier to add on every 1 damage to boss", _, true, 0.0, true, 1.0);
	g_ConfigConvar.Create("vsh_dome_speed_recover", "0.2", "Amount of slowdown per second to recover fom vsh_dome_speed_death and vsh_dome_speed_damage", _, true, 0.0, true, 1.0);
}
...
void Dome_PlayerDeath()
{
	if (g_flDomeStart == 0.0 || g_nDomeTeamOwner == TFTeam_Boss)
		return;
	
	g_flDomeSlowdown += g_ConfigConvar.LookupFloat("vsh_dome_speed_death");
	if (g_flDomeSlowdown > 1.0)
		g_flDomeSlowdown = 1.0;
}

void Dome_PlayerHurt(int iVictim, int iAttacker, int iDamage)
{
	if (g_flDomeStart == 0.0 || g_nDomeTeamOwner == TFTeam_Attack)
		return;
	
	if (!SaxtonHale_IsValidBoss(iVictim, false))
		return;
	
	if (iAttacker <= 0 || iAttacker > MaxClients || !IsClientInGame(iAttacker) || TF2_GetClientTeam(iAttacker) != TFTeam_Attack)
		return;
	
	g_flDomeSlowdown += g_ConfigConvar.LookupFloat("vsh_dome_speed_damage") * iDamage;
	if (g_flDomeSlowdown > 1.0)
		g_flDomeSlowdown = 1.0;
}
...
void Dome_UpdateRadius()
{
	...
	//Calculate dome current progress for flRadiusPrecentage (1.0 if started, 0.0 if finished shrinking)
	...
	float flRadiusPrecentage = (g_flDomeRadius - flRadiusEnd) / flRadiusDistance;
	
	//Get speed convar values
	float flSpeedDuration = g_ConfigConvar.LookupFloat("vsh_dome_speed_duration");
	float flSpeedEnd = g_ConfigConvar.LookupFloat("vsh_dome_speed_end");
	float flSpeedRecover = g_ConfigConvar.LookupFloat("vsh_dome_speed_recover");
	
	//Calculate speed dome should be
	float flSpeedAverage = flRadiusDistance / flSpeedDuration;
	float flSpeedMultiplier = flRadiusPrecentage * 2.0 * flSpeedEnd;
	float flSpeed = flSpeedAverage * flSpeedMultiplier * (1.0 - g_flDomeSlowdown);
	...
	//Update slowdown from recover
	g_flDomeSlowdown -= flSpeedRecover * flGameTimeDifference;
	if (g_flDomeSlowdown < 0.0)
		g_flDomeSlowdown = 0.0;
	
	//PrintToConsoleAll("flSpeedAverage (%.2f) flSpeedMultiplier (%.2f) g_flDomeSlowdown (%.2f) flSpeed (%.2f)", flSpeedAverage, flSpeedMultiplier, g_flDomeSlowdown, flSpeed);
}
